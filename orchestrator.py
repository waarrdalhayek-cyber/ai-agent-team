...
- business: travel, flights, hotels, restaurants, reservations, e-commerce, products, market/competitor analysis, stores, advertising, customer/order workflows
...
"""
SYNTHESIS_SYSTEM_PROMPT = """You are the GENERAL MANAGER. Combine the division manager and specialist results into one concise final result. Never claim an external action, booking, payment, purchase, calendar write, generation, or transaction occurred unless confirmed by the executing specialist. Financial actions always require the user's explicit approval for that specific action."""
...
class Orchestrator:
    ...
    def route(self, task: str, agent_name: str) -> str:
        """Backward-compatible direct specialist route used by existing smoke tests/tools."""
    ...
    def synthesize(self, task: str, results: dict[str, str]) -> str:
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[{"role": "system", "content": SYNTHESIS_SYSTEM_PROMPT}, {"role": "user", "content": f"Original Task: {task}\n\nResults:\n{packed}"}],
        )
