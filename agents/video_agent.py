...
class VideoAgent(BaseAgent):
    ...
    def _parse_plan(plan_text: str) -> dict[str, Any]:
        """Accept strict JSON, fenced JSON, or gracefully preserve planner text."""
        ...
        for candidate in candidates:
            try:
                ...
                pass
        return {"planner_text": text, "parser_warning": "Planner response was not valid JSON; local production continued."}
    ...
    def _render_local_preview(self) -> dict[str, Any]:
        ...
        if image is None: missing.append("image_asset")
        if audio is None: missing.append("audio_asset")
        if missing:
            return {"status": "LOCAL_RENDER_UNAVAILABLE", "missing": missing, "scene_count": len(scenes)}

        output_dir = Path("outputs/video")
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / "local_preview.mp4"
        video_filter = "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,zoompan=z='min(zoom+0.0008,1.12)':d=1:s=1080x1920:fps=30"
        command = [ffmpeg, "-y", "-loop", "1", "-i", str(image), "-i", str(audio), "-vf", video_filter, "-c:v", "libx264", "-preset", "medium", "-crf", "21", "-c:a", "aac", "-b:a", "192k", "-pix_fmt", "yuv420p", "-shortest", "-movflags", "+faststart", str(output_path)]
        completed = subprocess.run(command, capture_output=True, text=True, encoding="utf-8", errors="replace", check=False)
        if completed.returncode != 0:
            return {"status": "LOCAL_RENDER_FAILED", "returncode": completed.returncode, "stderr_tail": completed.stderr[-3000:], "scene_count": len(scenes)}
        return {"status": "LOCAL_PREVIEW_CREATED", "output_path": str(output_path), "visual_mode": "animated_reference_image", "format": "vertical_9_16", "scene_count": len(scenes), "scene_plan_loaded": bool(scenes), "paid_credits_used": False}
    ...
    def run(self, task: str, collaboration_context: str | None = None) -> str:
        ...
        manifest = {
            "status": "PRODUCTION_ATTEMPTED", "agent": "video", "request": task, "plan": plan,
            "available_assets": self._find_assets(), "local_execution": local_execution,
            "spending_policy": {"priority": ["existing_assets", "free_credits", "local_tools", "paid_provider"], "paid_generation_authorized": self._paid_generation_authorized(), "rule": "Paid generation is blocked unless explicitly authorized."},
        }
        output_dir = Path("outputs/video")
        output_dir.mkdir(parents=True, exist_ok=True)
