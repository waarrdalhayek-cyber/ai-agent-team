...
class VideoAgent(BaseAgent):
    ...
    def _write_scene1_ass(self, video: Path, lyrics: str, ass_path: Path) -> list[str]:
        ...
        for index, line in enumerate(lines):
            ...
            events.append(f"Dialogue: 0,{self._ass_time(start)},{self._ass_time(end)},Arabic,,0,0,0,,{self._ass_escape(line)}")
        ass = "\n".join([
    ...
    def _finish_test_scene_locally(self) -> dict[str, Any]:
        ...
        if not video.is_file(): return {"status": "GENERATED_VIDEO_MISSING", "paid_credits_used": False}
        if audio is None: return {"status": "AUDIO_ASSET_MISSING", "paid_credits_used": False}
        lyrics = str(scenes[0]["lyrics"]) if scenes and scenes[0].get("lyrics") else "آداب الحديث"
        output_dir = Path("outputs/video"); output_dir.mkdir(parents=True, exist_ok=True)
        ...
        command = ["ffmpeg", "-y", "-i", str(video), "-i", str(audio), "-vf", f"ass='{ass_path.as_posix()}'", "-map", "0:v:0", "-map", "1:a:0", "-c:v", "libx264", "-preset", "medium", "-crf", "20", "-c:a", "aac", "-b:a", "192k", "-shortest", str(output)]
        try: proc = subprocess.run(command, capture_output=True, text=True, encoding="utf-8", errors="replace")
        except FileNotFoundError: return {"status": "FFMPEG_MISSING", "paid_credits_used": False}
        if proc.returncode != 0: return {"status": "LOCAL_FINISH_FAILED", "error": proc.stderr[-3000:], "paid_credits_used": False}
        return {"status": "FINISHED_TEST_SCENE_CREATED", "output_path": str(output), "source_video": str(video), "audio_path": str(audio), "subtitle_lines": subtitle_lines, "subtitle_mode": "sequential_ass_arabic_center_tiktok_safe", "paid_credits_used": False}

    def _generate_scene(self, scene_number: int) -> dict[str, Any]:
        """Generate exactly one requested scene. Financial gate remains in provider."""
        ...
        if scene_number < 1 or scene_number > len(scenes):
            return {"status": "SCENE_NOT_FOUND", "scene_number": scene_number, "paid_credits_used": False}
        scene = scenes[scene_number - 1]
        prompt = f"{scene['prompt']} Natural subtle movement, stable characters, preserve faces and clothing, no text."
        output = Path(f"outputs/video/replicate_scene_{scene_number:02d}.mp4")
        if output.is_file():
            return {"status": "REUSING_EXISTING_AI_VIDEO", "scene_number": scene_number, "output_path": str(output), "paid_credits_used": False}
        result = generate_image_to_video(prompt, image, output)
        result["scene_number"] = scene_number
        return result

    def _generate_real_test_scene(self) -> dict[str, Any]:
        return self._generate_scene(1)

    def _generate_second_scene_test(self) -> dict[str, Any]:
        """Generate only scene 2; used for the separately approved single-scene test."""
        return self._generate_scene(2)
    ...
    def run(self, task: str, collaboration_context: str | None = None) -> str:
        ...
        execution = {"status": "REUSING_EXISTING_AI_VIDEO", "output_path": str(existing_video), "paid_credits_used": False} if existing_video.is_file() else self._generate_real_test_scene()
        ...
        manifest = {"status": "PRODUCTION_ATTEMPTED", "agent": "video", "request": task, "plan": plan, "available_assets": self._find_assets(), "real_video_execution": execution, "local_finish": local_finish, "spending_policy": {"paid_generation_authorized": self._paid_generation_authorized(), "rule": "Replicate inference is blocked unless explicit financial approval is enabled for this task. Existing generated clips are reused without spending."}}
        out = Path("outputs/video"); out.mkdir(parents=True, exist_ok=True)
